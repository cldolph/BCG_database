...
# Data from EPA Project Round 1 

Data from Upper Mississippi-Ohio-Tennessee River Basins.
...
# Data from EPA Project Round 2

Assembled by Virginia & Christy. BCG "proxies" for these datasets were
assigned according to methods described in Bonacquist-Currin et al. (in
prep).
...
Download NHD flowlines for the study area using nhdplusTools, which
access the NHDPlusv2 data.
...
#Unzip HUC12 geodatabase in spatial data folder 
unzip(paste0(dir$data_huc, '/HUC_spatial_data.gdb.zip'), exdir=dir$data_huc)
...
#Obtain HUC4 IDs from HUC12 and select desired columns
BCG_clean<-EPA.NHD.HUC12 %>% 
  st_drop_geometry() %>%
  dplyr::bind_cols(coord_df) %>%  
  mutate(HUC4=str_sub(huc12, 1, 4)) %>% 
  select(State, SiteID, Date, Lat, Long, Taxa, BCG_proxy, huc12_name,
          areasqkm, comid, gnis_id, gnis_name, lengthkm, reachcode, ftype, fcode, streamleve,
          streamorde, HUC4, huc12)

head(BCG_clean)

#Write updated data to table:
write.csv(BCG_clean, paste(dir$output, "/EPA_BCG_allsamples_ALLROUNDS_NHDCOMID_clean_HUC12.csv", sep=""),
          row.names=FALSE)
```
