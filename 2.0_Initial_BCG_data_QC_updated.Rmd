...
#~10% of all rows are multi samples per date (this might be more applicable for river samples)
...
#The issue is, if some sites have many samples per year, that will skew the average HUC8 average towards the BCG values at those sites
#Same issue will occur if some sites are sampled repeatedly over the years; but later will select most recent yearly sample for every site
  #create average BCG score for each site, every year
...
#now add counts per site
  add_count(LatLong_ID, name = "total_samples_per_site")
